...
var showInspiration = function(item) {
	...
	var user = result.find('.user a')
		...
		.html(item.user.display_name + "<br>");
    var image = "<img src='" + item.user.profile_image + "' alt='" + item.user.display_name + "' class='img-thumbnail'>";
    $(user).append(image);
	result.find('.post-count').html(item.post_count);
	result.find('.score').html(item.score);
